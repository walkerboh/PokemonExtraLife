...
namespace PokemonExtraLifeApiCore.EntityFramework
{
    public class DonationProcessor
    {
        ...
        public (Donation, Pokemon, Pokemon, Trainer, Trainer, Host, Host) GetNextDonation()
        {

            Donation nextDonation = _context.Donations.OrderBy(d => d.Time).FirstOrDefault(d => !d.Processed);

            List<PokemonOrder> pokemonOrders = _context.PokemonOrders.Include("Pokemon").Include("Trainer.PokemonOrders.Pokemon").ToList();
            ...
            Host nextHost = null;
            Host currentHost = _context.Hosts.First(h => h.Id == displayStatus.CurrentHostId);
            ...
            if (nextDonation != null)
            {
                ...
                Group activeGroup = _context.ActiveGroup;
